...
class gcal : public Network {
    ...
    void preLoadPatterns(string filename){
        ...
        int nPat = tmp.size()/HCM.C.n;
        PreLoadedPatterns.resize(nPat,vector<double>(HCM.C.n,0.));
        int k=0;
        for(int i=0;i<nPat;i++){
            for(int j=0;j<HCM.C.n;j++){
...
int main(int argc, char **argv){
    ...
    Net.preLoadPatterns("configs/testPatterns.h5");
